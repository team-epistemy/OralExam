...
def migrate() -> None:
    ...
    with conn.cursor() as cur:
        ...
        # Always apply migration_010 (class_session table; idempotent)
        migration_010 = db_dir / "migration_010_class_sessions.sql"
        ...
        # Always apply migration_017 (agent_simulation table + grant; idempotent)
        migration_017 = db_dir / "migration_017_agent_simulation.sql"
        if migration_017.exists():
        ...
        # Always apply migration_019 (qg_test_run bench history + grant; idempotent)
        migration_019 = db_dir / "migration_019_qg_test_run.sql"
        if migration_019.exists():
        ...
        # Always apply migration_021 (perf_probe records + grant; idempotent)
        migration_021 = db_dir / "migration_021_perf_probe.sql"
        if migration_021.exists():
        ...
        # Always apply migration_025 (examiner_config text_first toggle; idempotent)
        migration_025 = db_dir / "migration_025_text_first.sql"
        if migration_025.exists():
