...
_GRAPH_EXTRACTION_PROMPT = (
    ...
    "for dense or technical material, capture the underlying ideas and reasoning — "
    "not surface formulas or numbers for their own sake. For EACH concept also author "
    "a DEPTH-TAGGED question bank grounded in the material's ideas, with these tiers: "
    "'recall' = 2 questions on the concept's core meaning — key definitions and "
    "essential facts in the student's own words (NOT rote formula or number "
    "reproduction); 'application' = 2 questions that give a brief concrete situation "
    "and ask the student to apply the concept to it; 'in_depth' = 2 higher-order "
    "'why/how' questions on mechanisms, prerequisite chains, or multi-step reasoning; "
    "'case' = 1 question that opens with a brief 1-2 sentence mini-case (a realistic "
    "scenario from the material's domain) and asks the student to APPLY the concept "
    "to it. Every question MUST be self-contained: it carries its own context and is "
    "answerable without having the source in front of you. NEVER reference the "
    "material's structure or location (e.g. 'in Module 4', 'according to the reading', "
    "'the slides/textbook') and NEVER ask the student to 'list' or enumerate a fixed "
    "number of items — probe understanding, not recall of a bulleted list. Favor "
    "conceptual understanding over low-level arithmetic. Also identify "
...
def _generate_concept_banks(settings, concepts: list, relations: list, difficulty: str) -> dict:
    ...
    system_prompt = (
        ...
        "them aloud. Each question is self-contained — it carries its own context and NEVER "
        "references the material's structure or location ('in Module 4', 'the reading', 'the "
        "slides') and NEVER asks the student to 'list' a fixed number of items. Favor conceptual "
        "understanding over low-level arithmetic. Do NOT invent facts beyond the graph, and do "
        "NOT use a generic template. "
        "Return ONLY valid JSON, no prose, no markdown fences: "
