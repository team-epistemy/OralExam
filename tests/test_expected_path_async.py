...
def test_await_stops_as_soon_as_the_filler_gives_up(monkeypatch):
    """A dead filler must end the wait, not leave the turn sitting out the budget.

    An empty path reads the same whether the worker is about to commit or has
    already failed, so the in-flight set is the only signal. A bad API key fails
    in ~0.3s; without this check all five sub-turns of every question would each
    still burn the remaining ~5.7s waiting for a thread that is gone.
    """
    qid = "d" * 36

    def fake_read(_repo, _qid):
        reads.append(_qid)
        http_app._INFLIGHT_PATHS.discard(qid)   # worker dies right after poll 1
        return {}
    ...
    monkeypatch.setattr(http_app.time, "sleep",
                        lambda s: pytest.fail("should not have slept"))
    http_app._INFLIGHT_PATHS.add(qid)
    try:
        assert http_app._await_expected_path(object(), qid, 6.0) == {}
    finally:
        http_app._INFLIGHT_PATHS.discard(qid)
    assert len(reads) == 1
